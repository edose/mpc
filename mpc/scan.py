...
MIN_ALTITUDE = 30
MAX_SUN_ALTITUDE = -12
MAX_V_MAG = 20  # ~ optimistic for 3-min exposure, but this is through a clear filter (not V).
MIN_UNCERTAINTY = 2  # minimum orbit uncertainty to consider following up (in arcseconds).
MIN_MOON_DIST = 30


def get_candidates(mp_start=100000, date='20181125', max_mps=10000, max_candidates=100):
    all_dict_list = []
    html_base = mp_start
    last_mp = mp_start + max_mps - 1
    while html_base <= last_mp and len(all_dict_list) < max_candidates:
        n = min(last_mp - html_base + 1, MAX_MP_PER_HTML)
        lines = get_one_html(start=mp_start, n=n, date=date)
        ...
        if len(line_split) >= MIN_TABLE_WORDS:
            ...
            if this_mp_alt_is_max_so_far:
                ...
                if mp_alt >= MIN_ALTITUDE and v_mag <= MAX_V_MAG and sun_alt <= MAX_SUN_ALTITUDE and \
                    ...
                    uncertainty = get_uncertainty(line_split)  # subordinate test as it causes a web call.
...
def get_uncertainty(line_split):
    url = line_split[-1].split('href=\"')[-1].split('&OC')[0]
    sleep(0.5 * randint(1, 3))  # delay between calls (playing nicely with MPC server).
